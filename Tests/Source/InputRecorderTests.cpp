...
BOOST_AUTO_TEST_SUITE(InputRecorderTests)
	...
	struct InputHandlerConsumeEventFixture
	{
		...
		using TestRecorder = InputRecorder<KeyEventComparitor, AnyEventFilter>;
...
BOOST_FIXTURE_TEST_CASE(CompletingGestureFiresActionForMultiInputGesture, InputHandlerConsumeEventFixture)
{
	...
	TestGestureBind bind({ upPressed, downPressed }, [&actionFired]() { actionFired = true; });

	inputRecorder.handleEvent(0, upPressed);
	inputRecorder.handleEvent(0, downPressed);
	TestGestureBind testBind = inputRecorder.GetCompletedBind([&actionFired]() { actionFired = true; });

	const std::vector<sf::Event>& returnVec = testBind.getGesture();
	const std::vector<sf::Event>& testVec = bind.getGesture();
	for (int ii = 0; ii < returnVec.size(); ii++)
	{
			BOOST_TEST(std::invoke(inputRecorder.m_eventComparitor, returnVec[ii], testVec[ii]));
	}
